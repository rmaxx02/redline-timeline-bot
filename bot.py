...
async def on_ready():
    ...
    if log_ch:
        await log_ch.send(
            "📟 **SYSTEM ONLINE:** Upgraded Chronological Forum Router running successfully."
        )
...
async def status_rotator():
    statuses = [
        discord.Activity(
            type=discord.ActivityType.watching, name="🏎️ Opie's POV (Driver)"
        ),
        discord.Activity(
            type=discord.ActivityType.watching, name="💻 Tray's POV (Hacker)"
        ),
        discord.Activity(
            type=discord.ActivityType.watching, name="🚓 Frenchie's POV (Recon)"
        ),
...
async def on_member_join(member):
    ...
    if welcome_ch:
        ...
        embed.set_footer(
            text=f"Redline Operative #{len(member.guild.members)} | Grid Sync Active"
        )
...
async def stats(ctx, *, option: str = None):
    if option and option.lower() == "leaderboard":
        if not USER_DATABASE:
            await ctx.send(
                "📊 **Scoreboard Empty:** No clips have been logged in the archive yet!"
            )
            return
        ...
        for uid, data in USER_DATABASE.items():
            total_clips = (
                data.get("Opie", 0) + data.get("Tray", 0) + data.get("Frenchie", 0)
            )
        ...
        embed = discord.Embed(
            title="🏆 REDLINE OVERALL CLIPS LEADERBOARD 🏆",
            description="The server's top verified chronological timeline contributors.",
            color=0xD4AF37,
        )
        for i, (name, total, opie_pts, tray_pts, frenchie_pts) in enumerate(
            leaderboard_data[:5], 1
        ):
            embed.add_field(
                name=f"🥇 Rank #{i} | {name}",
        ...
        embed.set_footer(
            text="Keep submitting clips to claim the top rank position on the dashboard!"
        )
    ...
    embed = discord.Embed(
        ...
        description="Your verified milestone submissions.",
        color=0xE67E22,
    )
    embed.add_field(
        name="🏎️ Opie (Driver Track)",
        value=f"Submissions: **{user_data['Opie']}**",
        inline=True,
    )
    embed.add_field(
        name="💻 Tray (Hacker Track)",
        value=f"Submissions: **{user_data['Tray']}**",
        inline=True,
    )
    embed.add_field(
        name="🚓 Frenchie (Recon Track)",
        value=f"Submissions: **{user_data['Frenchie']}**",
        inline=True,
    )
    embed.set_footer(
        text="Type '!stats leaderboard' to see the server's top 5 contributors!"
    )
    await ctx.send(embed=embed)
    ...
    if yt_match:
        ...
        if current_time - SPAM_COOLDOWN.get(uid, 0) < 5:
            await msg.channel.send(
                f"⚠️ {msg.author.mention}, slow down!", delete_after=3
            )
            try:
                await msg.delete()
            except:
        ...
        if video_id in PROCESSED_VIDEOS_CACHE:
            try:
                ...
                pass
            await msg.channel.send(
                f"❌ {msg.author.mention} **Submission Blocked:** That specific video clip is already logged in the archive tracker dashboard!",
                delete_after=10,
            )
            ...
            if log_ch:
                await log_ch.send(
                    f"⚠️ **DUPLICATE INTERCEPTED:** {msg.author.mention} attempted to submit a duplicate link. Video ID: `{video_id}`. Message deleted automatically."
                )
            return
        ...
        tagged_players_output_string = (
            " ".join(detected_player_tags)
            if detected_player_tags
            else "*No matching streamer names identified inside description blocks*"
        )

        combined_metadata_text = f"{content_lower} {actual_video_title.lower()}"

        if any(word in combined_metadata_text for word in GANG_WAR_WORDS):
            tag_label, target_tag_name, embed_color = (
                "🔴 GANG WAR LOG",
                "🔴 Gang War",
                0xFF0000,
            )
        elif any(word in combined_metadata_text for word in HEIST_WORDS):
            tag_label, target_tag_name, embed_color = (
                "💰 ACTIVE HEIST TIMELINE",
                "💰 Heist",
                0x39FF14,
            )
        ...
        tracked_streamer, streamer_tag, track_key, target_streamer_tag_name = (
            None,
            "Unknown Operative",
            None,
            None,
        ...
        if "Opie fan" in roles_found:
            ...
            track_key, tracked_streamer = "Opie", (
                "Opie's Driver Track",
                USER_DATABASE[uid]["Opie"],
            )
            ...
            track_key, tracked_streamer = "Tray", (
                "Tray Sander's Hacker Track",
                USER_DATABASE[uid]["Tray"],
            )
            ...
            track_key, tracked_streamer = "Frenchie", (
                "Frenchie's Recon Track",
                USER_DATABASE[uid]["Frenchie"],
            )
            streamer_tag = (
                "[🚓 Frenchie (Recon Track)](https://www.youtube.com/@Frenchie)"
            )
            target_streamer_tag_name = "🚓 Frenchie"

        user_embed = discord.Embed(title=f"{tag_label} DETECTED", color=embed_color)
        user_embed.add_field(
            name="🎬 Track Perspective", value=streamer_tag, inline=True
        )
        user_embed.add_field(
            name="📅 YouTube Upload Date",
            value=f"📆 **{youtube_upload_date_string}**",
            inline=True,
        )
        user_embed.add_field(
            name="👥 Auto-Tagged Players In Video",
            value=tagged_players_output_string,
            inline=False,
        )
        ...
        if tracked_streamer:
            ...
            user_embed.add_field(
                ...
                value=f"{track_title}: **{track_count} clips**",
                inline=True,
            )
                ...
                if track_count >= milestone:
                    if rank_name not in roles_found:
                        ...
                        if role:
                            ...
                            await msg.channel.send(
                                f"⚡ **RANK UP:** {member.mention} has leveled up to **{rank_name}**! 🟢"
                            )
                    break
        ...
        if forum_channel and isinstance(forum_channel, discord.ForumChannel):
            applied_tags = [
                t
                for t in forum_channel.available_tags
                if t.name
                in [target_tag_name, target_streamer_tag_name, target_year_tag_name]
            ]

            clean_streamer_name = (
                target_streamer_tag_name.replace("🏎️ ", "")
                .replace("💻 ", "")
                .replace("🚓 ", "")
            ...
            await forum_channel.create_thread(
                name=thread_title, embed=user_embed, applied_tags=applied_tags
            )

            log_ch = bot.get_channel(LOG_ID)
            if log_ch:
                await log_ch.send(
                    f"✅ **CHRONO CARD ACTIVE:** Successfully created archive thread: `{thread_title}` for member {msg.author.mention}. AI tags parsed."
                )
